...
snd_pcm_uframes_t cg_frames = 128;
...
void InitMusic(char *musicfilename){
   ...
   int                   dir = 0;
   int                   err;
   unsigned int btime = 500000;/* I do not known why but this setting is always work */
   unsigned int ptime = 100000;/* I do not known why but this setting is always work */
   ...
   snd_pcm_hw_params_set_period_time_near(cg_audio, hw_params, &ptime, &dir);
   snd_pcm_hw_params_set_buffer_time_near(cg_audio, hw_params, &btime, &dir);
...
void AsyncPlay(){
   ...
   /*
      We need this for cicle to send enought data to the callback.
